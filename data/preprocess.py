...
if __name__ == "__main__":
    data = pd.read_csv("data/data/sz50/sz50.csv", index_col=0)
    data = data.rename(columns={'time': 'date', "code": "tic"})
    data["adjcp"] = data["close"].copy()
    data.index = data.groupby("date").ngroup()
    data = (generate_normalized_feature(data))
    train, valid, test = split(data, [0.8, 0.1, 0.1])
    train.to_csv("data/data/sz50/train.csv")
    valid.to_csv("data/data/sz50/valid.csv")
    test.to_csv("data/data/sz50/test.csv")
